...
def _vehicles_df(dets_df: pd.DataFrame) -> pd.DataFrame:
    """Return only vehicle detections (trucks, cars, buses, trains)."""
...
def _persons_df(dets_df: pd.DataFrame) -> pd.DataFrame:
    """Return only person detections."""
    if dets_df is None or dets_df.empty:
        return pd.DataFrame()
    if "cls_name" in dets_df.columns:
        return dets_df[dets_df["cls_name"] == "person"].copy()
    return pd.DataFrame()


def _taggable_assets_df(dets_df: pd.DataFrame) -> pd.DataFrame:
    """Return all taggable assets (vehicles + persons)."""
    if dets_df is None or dets_df.empty:
        return pd.DataFrame()
    if "cls_name" in dets_df.columns:
        return dets_df[dets_df["cls_name"].isin(["truck", "car", "bus", "train", "person"])].copy()
    return dets_df.copy()
...
def _update_role_memory(dets_df: pd.DataFrame, now_t: float):
    ...
    by_tid = {int(r["track_id"]): tuple(r["bbox_xyxy"]) for _, r in assets.iterrows()}
...
def _role_handoff(dets_df: pd.DataFrame, now_t: float, iou_thr: float = 0.20, max_age_sec: float = 8.0):
    ...
    present_tids = set(int(x) for x in assets["track_id"].dropna().tolist())
    present_roles = set()
...
def render_asset_tagging(dets_df: pd.DataFrame):
    ...
    validation_warnings = validate_asset_tagging(dets_df)
    ...
    # Split detections into vehicles and persons
    if "cls_name" in dets_df.columns:
        vehicles = dets_df[dets_df["cls_name"].isin(["truck", "car", "bus", "train"])].copy()
        persons = dets_df[dets_df["cls_name"] == "person"].copy()
    else:
        vehicles = dets_df.copy()
        persons = pd.DataFrame()

    # Sort by confidence
    if not vehicles.empty and "conf" in vehicles.columns:
        vehicles = vehicles.sort_values("conf", ascending=False)
    if not persons.empty and "conf" in persons.columns:
        persons = persons.sort_values("conf", ascending=False)

    # One row per track_id: keep best conf (robust)
    if not vehicles.empty and "track_id" in vehicles.columns:
        if "conf" in vehicles.columns:
            vehicles = (
                vehicles.sort_values("conf", ascending=False)
                .groupby("track_id", as_index=False)
                .first()
            )
        else:
            vehicles = vehicles.groupby("track_id", as_index=False).first()

    if not persons.empty and "track_id" in persons.columns:
        if "conf" in persons.columns:
            persons = (
                persons.sort_values("conf", ascending=False)
                .groupby("track_id", as_index=False)
                .first()
            )
        else:
            persons = persons.groupby("track_id", as_index=False).first()
    ...
    if not vehicles.empty:
        ...
        for _, r in vehicles.iterrows():
            ...
            with c2:
                # Only show vehicle roles for vehicles
                vehicle_role_options = ["UNASSIGNED"] + [k for k in ROLE_OPTIONS.keys() if k in VEHICLE_ROLES]
                current_idx = vehicle_role_options.index(current) if current in vehicle_role_options else 0

                new_role = st.selectbox(
                    label="",
                    options=vehicle_role_options,
        ...
        st.info("No vehicle-class detections (truck/car/bus/train).")
    ...
    if not persons.empty:
        ...
        for _, r in persons.iterrows():
            tid = int(r["track_id"]) if "track_id" in r else -1
            cls_name = str(r.get("cls_name", "person"))
            conf = float(r.get("conf", 0.0))
            ...
            with c2:
                # Only show person roles for persons
                person_role_options = ["UNASSIGNED"] + [k for k in ROLE_OPTIONS.keys() if k in PERSON_ROLES]
                current_idx = person_role_options.index(current) if current in person_role_options else 0
        ...
        st.info("No person detections in current frame.")


def validate_asset_tagging(dets_df: Optional[pd.DataFrame] = None) -> List[Dict[str, str]]:
    """
    Validate current asset tagging and return list of warnings.

    Args:
        dets_df: Current detections DataFrame (optional, for mismatch checking)
    ...
    # VEHICLE VALIDATION: Check for duplicate critical vehicle roles (only 1 allowed)
    UNIQUE_VEHICLE_ROLES = ["FUEL_TRUCK", "GPU_TRUCK", "PUSHBACK_TUG"]
    for role in UNIQUE_VEHICLE_ROLES:
    ...
    # PERSON VALIDATION: Check for too many operators (warning only - multiple may be OK)
    operator_roles = ["GPU_OPERATOR", "FUEL_OPERATOR", "BAGGAGE_HANDLER"]
    for role in operator_roles:
        count = role_counts.get(role, 0)
        if count > 3:
            warnings.append({
                "severity": "WARNING",
                "message": f"Many {ROLE_OPTIONS.get(role, role)} tagged ({count}x) - verify if correct"
            })

    # Check for excessive passengers (info only)
    passenger_count = role_counts.get("PASSENGER", 0)
    if passenger_count > 20:
        warnings.append({
            "severity": "WARNING",
            "message": f"Many passengers tagged ({passenger_count}x) - this is normal for boarding/deboarding"
        })

    # CRITICAL: Check for role/detection type mismatches
    # If current detections show a person with a vehicle role (or vice versa), this is a bug
    if dets_df is not None and not dets_df.empty:
        if "track_id" in dets_df.columns and "cls_name" in dets_df.columns:
            for tid, role in st.session_state.asset_roles.items():
                ...
                # Find this track_id in current detections
                matching = dets_df[dets_df["track_id"] == tid]
                if matching.empty:
                    continue

                cls_name = matching.iloc[0]["cls_name"]
                is_person_detection = (cls_name == "person")
                is_vehicle_detection = (cls_name in ["truck", "car", "bus", "train"])

                # Check for mismatches
                if is_person_detection and role in VEHICLE_ROLES:
                    warnings.append({
                        "severity": "ERROR",
                        "message": f"Person detection (id={tid}) tagged with vehicle role '{ROLE_OPTIONS.get(role, role)}' - please re-tag!"
                    })
                elif is_vehicle_detection and role in PERSON_ROLES:
                    warnings.append({
                        "severity": "ERROR",
                        "message": f"Vehicle detection (id={tid}) tagged with person role '{ROLE_OPTIONS.get(role, role)}' - please re-tag!"
                    })
